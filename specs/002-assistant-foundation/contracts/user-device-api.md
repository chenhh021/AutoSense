...
**Date**: 2026-09-07
**Feature**: [002](../spec.md)
**Status**: 保留既有用户、权限和设备管理接口作为兼容基线；2026-09-15 新增工作流接入边界，旧实施证据见 validation.md，不将其视为新图已经完成。
...
自然语言设备查询、诊断取证及控制后复检由004的独立Query步骤执行，均先完成用户确认；图节点不得通过直接调用GET /devices或绑定接口来绕过步骤确认。为生成批准说明而读取本地身份、归属及候选标签属于安全解析，不进行外部设备探测。
本表保留原有确定性管理API的URL、DTO及错误语义；本轮不将页面加载或后台轮询解释为对话计划的批准。前端对话查询接入使用带approvalId的工作流批准接口，具体见 assistant-api.md；原绑定行为不能被LLM作为设备查询或控制工具自动调用。
