...
class WorkflowCheckpointIT extends AbstractIntegrationIT {
    ...
    @Test void persistsLatestAndSpecifiedSnapshotsAndRejectsAnOldExecutionFence() throws Exception {
        ...
        workflow.setOriginMessageId(9L); workflow.setLatestInputMessageId(9L); workflow.setGraphVersion(AssistantStateSerializer.GRAPH_VERSION);
        workflow.setSchemaVersion(AssistantStateSerializer.SCHEMA_VERSION); workflow.setStatus("CREATED"); workflow.setVersion(0L); workflow.setFence(0L);
        ...
        data.put(AssistantState.WORKFLOW, new AssistantState.WorkflowContext(w.status(), 0, w.progress(), 0, "", "", "", "", AssistantStateSerializer.SCHEMA_VERSION,
                AssistantStateSerializer.GRAPH_VERSION, 0, claim.fence()));
