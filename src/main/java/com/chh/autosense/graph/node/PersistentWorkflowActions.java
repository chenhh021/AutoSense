...
public final class PersistentWorkflowActions implements WorkflowStepActions {
    ...
    public boolean streamsAnswers() {
        return delegate.streamsAnswers();
    }
    ...
    public PlanProposal plan(AssistantState state, Duration remaining) throws Exception {
        return delegate.plan(state, remaining);
    }

    @Override
    public Map<String, Object> resolveTarget(AssistantState state) throws Exception {
        return delegate.resolveTarget(state);
    }

    @Override
    public void revalidate(AssistantState state) throws Exception {
        delegate.revalidate(state);
    }

    @Override
    public Map<String, Object> answer(AssistantState state, Duration remaining) throws Exception {
        return delegate.answer(state, remaining);
    }

    @Override
    public Map<String, Object> diagnose(AssistantState state, Duration remaining) throws Exception {
        return delegate.diagnose(state, remaining);
    }

    @Override
    public Map<String, Object> query(AssistantState state, Duration remaining) throws Exception {
        persistence.beginQuery(state);
        return delegate.query(state, remaining);
    }

    @Override
    public void prepareCommand(AssistantState state) {
        commands.prepare(state);
    }

    @Override
    public Map<String, Object> control(AssistantState state, Duration remaining) throws Exception {
    ...
    public void commitResult(AssistantState state, ExecutionPlan.Result result) {
        persistence.commitResult(state, result);
    }

    @Override
    public boolean supportsSafeCommandRetry() {
        return delegate.supportsSafeCommandRetry();
    }

    @Override
    public Map<String, Object> afterNode(AssistantState before, Map<String, Object> delta) {
        ...
        if (change.get(CONTROL) instanceof ControlContext control && control.approvalRef() != null
            ...
            payload.put("approvalId", reference.approvalId());
            payload.put("expiresAt", reference.expiresAt().toString());
