...
import com.chh.autosense.aftersales.AfterSalesGuideService;
import com.chh.autosense.aftersales.AfterSalesLocation;
import com.chh.autosense.analysis.DiagnosisConclusion;
import com.chh.autosense.analysis.DiagnosisReasoner;
import com.chh.autosense.analysis.ProblemAnalysis;
import com.chh.autosense.analysis.ProblemAnalyzer;
import com.chh.autosense.api.ApiException;
import com.chh.autosense.api.ErrorCode;
import com.chh.autosense.api.dto.ConclusionDto;
...
import com.chh.autosense.device.rule.FaultRuleEngine;
...
import com.chh.autosense.knowledge.RepairKnowledgeService;
import com.chh.autosense.repository.ChatMessageMapper;
import com.chh.autosense.repository.DiagnosticSnapshotMapper;
import com.chh.autosense.repository.ProblemReportMapper;
...
import com.chh.autosense.security.DeviceOwnershipChecker;
...
public class SessionOrchestrator {
        ...
        switch (intent) {
            case COMMON_SENSE, MODEL_SPECIFIC -> {
                ...
                directAnswerer.answer(text, session.getId(),
                                token -> stream.send(com.chh.autosense.api.sse.SseEvent.token(token)))
    ...
    private void completeDirectAnswer(RepairSession session, String answer, Throwable error,
        ...
        try {
            ...
            if (error != null || completedAnswer == null || completedAnswer.isBlank()) {
                ...
                stream.send(com.chh.autosense.api.sse.SseEvent.token(completedAnswer));
            }
    ...
    private void transition(RepairSession session, SessionStatus from, SessionStatus to,
        ...
        stream.send(com.chh.autosense.api.sse.SseEvent.status(session.getId(), to.name()));
    }
