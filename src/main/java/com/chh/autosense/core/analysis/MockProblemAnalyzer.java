 ...
 * 保证无真实模型时可走通全流程。生产必须切换为真实 AI Service 代理（章程原则 IV）。
 * 本期仅智能灯泡在支持清单；路由器/空调会被识别但遭拒识（FR-004）。
 */
@Component
...
public class MockProblemAnalyzer implements ProblemAnalyzer {
    ...
    public ProblemAnalysis analyze(String text, ConversationHistorySnapshot history) {
        ...
        String deviceType = detectDeviceType(input);
        if (deviceType == null && !input.isBlank()) {
            return new ProblemAnalysis(null, null, null, false,
                    "请问出问题的设备是什么类型？目前支持：智能灯泡。");
        }
        if (deviceType != null && input.length() < 6) {
            return new ProblemAnalysis(deviceType, null, null, false,
                    "能具体描述一下问题的表现吗？比如什么时候开始出现、如何复现？");
        }
        return new ProblemAnalysis(deviceType, input, null, deviceType != null, null);
    }
